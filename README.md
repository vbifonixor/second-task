...
-  `.checkLecture(lecture[, editor])` - возвращает true, если объект лекции, переданный в аргументе `lecture`, соответствует формату, описанном в секции [Конструктор](#constructor) и false, если не соответствует. Необязательный аргумент `editor`, если приводится к true, опускает проверку о существовании лекции с таким названием (для вызова методом-редактором).
...
-  `.checkSchool(school[, editor])` - возвращает true, если объект школы, переданный в аргументе `school`, соответствует формату, описанном в секции [Конструктор](#constructor) и false, если не соответствует. Необязательный аргумент `editor`, если приводится к true, опускает проверку о существовании школы с таким названием (для вызова методом-редактором).
...
-  `.checkPlace(place[, editor])` - возвращает true, если объект аудитории, переданный в аргументе `place`, соответствует формату, описанном в секции [Конструктор](#constructor) и false, если не соответствует. Необязательный аргумент `editor`, если приводится к true, опускает проверку о существовании аудитории с таким названием (для вызова методом-редактором).
